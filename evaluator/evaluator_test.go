...
func testBooleanObject(t *testing.T, obj object.Object, expected bool, input string) bool {
	...
	if result.Value != expected {
		t.Errorf("object has wrong value. input=%s got=%t, want=%t", input, result.Value, expected)
		return false
	}
	return true
}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function haws wrong parameters. Parameters=%+v", fn.Parameters)
	}
...
func TestBuiltInFunctions(t *testing.T) {
	tests := []struct {
		...
		{`rest([])`, NULL},
		{`rest([0])`, []int64{}},
		{`rest([0, 1, 2])`, []int64{1, 2}},
		{`push([], 10)`, []int64{10}},
		{`push([0], 10)`, []int64{0, 10}},
